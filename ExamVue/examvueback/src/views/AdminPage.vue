<template>
  <div>
    <h1>{{tittle}}</h1>
    <el-tooltip class="item" effect="dark" content="管理员默认密码:666666" placement="top-start">
    ...
    <el-table
      ...
      >
        <template slot-scope="rowdata" >
          ...
          <el-button v-if="rowdata.row!=null" type="primary" @click="delHandler(rowdata.row)">删除管理员</el-button>
    ...
    <el-dialog ref="infoShow" :visible.sync="isDialogShow">
      <el-form>
        <el-form-item   v-for="(item,index) in tabCol" :label="item.desc" :key="index"  v-if="item.prop!=='aid'&&item.prop!='apwd'">
          ...
          <el-select :disabled="isEdit" v-if="item.type==='select' && item.prop=='rid' " v-model="AObject[item.prop]">
            <el-option  :value="role.rid" :label="role.rname" v-for="(role,index) in innerRoles"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods:{
    ...
    ridRule(row,index){
      if (row!=null) {
        for (let i = 0; i < this.innerRoles.length; i++) {
          if (this.innerRoles[i].rid == row.rid) {
            return this.innerRoles[i].rname + ""
          }
        }
      }
    },
